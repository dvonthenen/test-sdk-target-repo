...
docker run --name ${CONTAINER_NAME} -t -v "${REPO_DIR}":/test-sdk-target-repo:ro cytopia/yamllint -s -c /test-sdk-target-repo/.yamllintconfig.yaml /test-sdk-target-repo
